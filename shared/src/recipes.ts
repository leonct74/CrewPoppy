...
export const RECIPES: Recipe[] = [
  ...
  },
  {
    ...
    instructions: [
      ...
      "To add an expense use your APPEND tool — one line, straight onto the end. Never read",
      "the whole file and write it back to add something: appending is far cheaper, and it",
      "cannot damage the expenses already recorded.",
      "If the file is new, append the header line first, then the expense.",
      "Use today's date (from your instructions above) when the owner says \"today\"; never",
      "invent a date. Amounts use a dot for decimals: 34.00.",
      "If your owner might have logged the same thing twice, read the file and check before",
      "appending — but only when there is real doubt, not routinely.",
      "",
  ...
  },
  {
    ...
    instructions: [
      ...
      "them). Use your APPEND tool to add the line — never rewrite the whole file to add a",
      "payment, or you risk mangling what is already recorded. Append the header first if",
      "the file is new. Use today's date unless told another.",
