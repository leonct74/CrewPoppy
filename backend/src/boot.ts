 ...
 *
 * Carried over from the VM-Poppy/TrafficPoppy lineage unchanged apart from the poppy
 * name in the error copy — it is the same broker contract, and the 202-envelope
 * handling is hard-won.
 */
export function brokerCredentialsProvider(
  ...
  const post = async (body?: unknown): Promise<Response> => {
    ...
    try {
      ...
      throw new Error(
        `CrewPoppy is waiting for AWS access from AgentsPoppy. If this doesn't clear, reopen AgentsPoppy and check CrewPoppy is connected. (${(e as Error).message})`,
      );
    }
  };
  ...
  async function mint(): Promise<AwsCredentialIdentity> {
    ...
    for (;;) {
      if (!res.ok) {
        let message = `AgentsPoppy won't grant AWS access right now (${res.status}). Check that CrewPoppy is connected and your AWS access is healthy in AgentsPoppy.`;
        try {
